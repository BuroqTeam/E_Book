...
OnChange?.Invoke();
            switch (axis) {
                ...
                case 0:
                    ...
                    tween = rect.DOAnchorPosX(pos.x, moveDuration).SetEase(Ease.OutElastic).SetDelay(StartDelay);
                    break;
                case 1:
                    dict = RectPositionYTweens;
                    tween = rect.DOAnchorPosY(pos.y, moveDuration).SetEase(Ease.OutElastic).SetDelay(StartDelay);
                    break;
                default:
                    return;
            }
